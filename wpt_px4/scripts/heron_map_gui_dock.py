...
class SensorFusionEKF(Node):
    def __init__(self):
        ...
        self.line_cbf, = self.ax1.plot([], [], 'b-', label='cbf')
        self.line_cbf1, = self.ax1.plot([], [], 'g-', label='cbf')
        ...
        hullWidth = 0.1 * size  # Width of each hull
        separation = 0.2 * size  # Distance between the two hulls
        bodyWidth = 0.2 * size  # Width of the body connecting the hulls
        ...
        self.cbf_sub = self.create_subscription(Float64MultiArray, '/cbf', self.cbf_callback, 10)
        self.waypoint_sub = self.create_subscription(Waypoint, "/waypoints", self.waypoints_callback, 10)
        self.control_sub = self.create_subscription(Float64MultiArray, "/control", self.control_callback, 10)
        ...
        self.a = 3.0
        self.a = 5.0
        self.x0 = (self.x0 - x_actual_min) / (x_actual_max - x_actual_min) * self.map_width
        self.y0 = self.map_height - (self.y0 - y_actual_min) / (y_actual_max - y_actual_min) * self.map_height
    ...
    def control_callback(self, msg):
        self.desired_r = msg.data[1]
        self.desired_u = msg.data[0]


    def cbf_callback(self, msg):  # - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        x_actual_min = 289577.66
        x_actual_max = 291591.05
        y_actual_min = 4117065.30
        y_actual_max = 4118523.52
        
        # self.x_cbf_data.clear
        # self.y_cbf_data.clear  
        self.x0 = (msg.data[0] - x_actual_min) / (x_actual_max - x_actual_min) * self.map_width
        self.y0 = self.map_height - (msg.data[1] - y_actual_min) / (y_actual_max - y_actual_min) * self.map_height
        self.a = msg.data[2]
        # self.a = msg.data[3]
    ...
    def ekf_callback(self, msg):  # - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        ...
        self.du_data.append(self.desired_u)
        self.dr_data.append(self.desired_r)

    def update_plot(self, frame):
        if len(self.time_data) > 1:
            self.line1_m.set_data(self.x_sensor_data, self.y_sensor_data)
            self.line1.set_data(self.x_data, self.y_data)
            # self.line_cbf.set_data(self.x_cbf_data, self.y_cbf_data)
            # self.line_cbf1.set_data(self.x_cbf_data, self.y1_cbf_data)
            x = np.linspace(self.x0 - 1.7, self.x0 + 1.7, 100)
            x1 = np.linspace(self.x0 - 5.0, self.x0 + 5.0, 100)  # Generate x values for plotting
            y = self.y0 + self.a * np.log((x - self.x0)**2 + 1)  # Calculate y based on the equation
            y1 = self.y0 + self.a * np.log((x1 - self.x0)**2 + 1)
            ...
            self.line_cbf.set_data(self.x_cbf_data, self.y_cbf_data)
            self.line_cbf1.set_data(self.x_cbf_data, self.y1_cbf_data)            
            # if self.x_cbf_data.size != 0 and self.y_cbf_data.size == self.x_cbf_data.size:
            #     self.line_cbf.set_data(self.x_cbf_data, self.y_cbf_data)
            # else:
            #     self.line_cbf.set_data([], [])

            # if self.x_cbf_data.size != 0 and self.y1_cbf_data.size == self.x_cbf_data.size:
            #     self.line_cbf1.set_data(self.x_cbf_data, self.y1_cbf_data)
            # else:
            #     self.line_cbf1.set_data([], [])

                
            # print(self.x_cbf_data)
            ...
            self.line8_left.set_data(self.time_data, self.du_data)
            self.line8_right.set_data(self.time_data, self.u_data)
            self.ax8.set_xlim(self.time_data[-1] - 20, self.time_data[-1])

            self.line9_left.set_data(self.time_data, self.dr_data)
            self.line9_right.set_data(self.time_data, self.r_data)
            ...
            return self.line1, self.line2, self.line3, self.line4, self.line5, self.line_cbf, self.line_cbf1
    ...
    def reset_plots(self, event):
        ...
        self.update_plot(None)  # Force a frame update
        self.fig.canvas.draw_idle()     
    # def run(self):
